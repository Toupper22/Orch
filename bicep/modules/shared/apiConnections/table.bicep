// Azure Table Storage API Connection Module
// Specialized module for creating Azure Table Storage API connections for Logic Apps
...
param displayName string = 'Azure Table Storage'
...
output id string = tableConnection.id

@description('API Connection name')
output name string = tableConnection.name
...
output connectionInfo object = {
  ...
  connectionName: tableConnection.name
